...
const STUDIO_BRIDGE = (typeof window !== 'undefined' && window.hermesDesktop?.studio) || null;
// Inside Basecamp we hit the upstream directly through the main-process proxy
// (studio:muapi:request), which has no CORS. A plain http(s) browser routes
// through the host's /api proxy; SSR / file:// call upstream directly.
const BASE_URL = STUDIO_BRIDGE
    ? 'https://api.muapi.ai'
...
// Transport shim: when running in Basecamp, proxy every request through the
// main process (bypasses CORS). Elsewhere it is a pass-through to global fetch.
// Shadowing the module-local `fetch` binding keeps all call sites below intact.
async function muFetch(url, init = {}) {
    if (!STUDIO_BRIDGE) return globalThis.fetch(url, init);
    const res = await STUDIO_BRIDGE.request({
        url: String(url),
...
export function uploadFile(apiKey, file, onProgress) {
    // In Basecamp, multipart upload also goes through the main-process proxy.
    // Progress is reported as a single 100% tick (no streaming over IPC).
    if (STUDIO_BRIDGE) {
        return (async () => {
            const bytes = await file.arrayBuffer();
            const res = await STUDIO_BRIDGE.upload({
                url: `${BASE_URL}/api/v1/upload_file`,
                apiKey,
                name: file.name,
                type: file.type,
                bytes
            ...
            if (!res.ok) {
                ...
                notifyAuthRequired(res.status, detail);

...
// prefilling non-English example prompts that ship with some Muapi templates.
  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-12 h-12 border-4 border-white/5 border-t-[#8b80e8] rounded-full animate-spin" />
...
function WorkflowCard({ workflow, onClick, activeTab, onRename, onDelete }) {
  ...
  // Directory-style card: no thumbnails at all, just organized text rows.
  return (
    <div
      onClick={() => onClick(workflow)}
      className="group relative flex flex-col justify-between rounded-xl overflow-hidden cursor-pointer border border-white/8 bg-[#0d0d28] p-4 min-h-[132px] transition-all hover:border-[#8b80e8]/40 hover:bg-[#12122e]"
    >
      {/* Left accent bar */}
      <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-[#8b80e8]/0 group-hover:bg-[#8b80e8]/70 transition-colors" />
      ...
      {activeTab === 'my-workflows' && (
        ...
        >
          <button
            ...
            className="w-8 h-8 rounded-full bg-black/30 border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-colors"
          >
          ...
          {showOptions && (
            <div className="absolute top-10 right-0 w-32 bg-[#111] border border-white/10 rounded-lg shadow-2xl py-1 animate-in fade-in zoom-in duration-200">
              <button
                ...
                className="w-full px-4 py-2 text-left text-[11px] font-bold text-white/70 hover:text-[#8b80e8] hover:bg-white/5 transition-colors flex items-center gap-2"
              >
      ...
      {/* Top: category */}
      <div className="text-[10px] font-bold text-[#8b80e8] uppercase tracking-wider opacity-80 pr-8">
        {workflow.category || "General"}
      </div>

      {/* Middle: name */}
      <h3 className="text-sm font-bold text-white leading-snug line-clamp-2 mt-2 group-hover:text-[#8b80e8] transition-colors">
        {workflow.name || "Untitled Flow"}
      </h3>

      {/* Bottom: author (published only) */}
      {activeTab === 'published' && workflow.user_name ? (
        <div className="flex items-center gap-2 mt-3">
          <img src={workflow.user_profile || "/user_profile.png"} alt="profile" onError={(e) => { e.currentTarget.src = "/user_profile.png"; }} className="w-4 h-4 rounded-full" />
          <span className="text-[9px] font-black text-white/50 uppercase tracking-widest truncate">{workflow.user_name}</span>
        </div>
      ) : (
        <div className="mt-3 text-[10px] font-semibold text-white/30 uppercase tracking-widest">
          Open workflow
        </div>
      )}
    </div>
...
export default function WorkflowStudio({ apiKey, isHeaderVisible = true, onToggleHeader }) {
        ...
        if (results[0].status === 'fulfilled') {
          ...
          Object.entries(schema.properties || {}).forEach(([key, prop]) => {
            const raw =
              ...
              "";
            // Some Muapi templates ship Hindi example prompts. Do not prefill
            // non-English text -- leave the field blank so the user types English.
            initial[key] = isEnglishOnly(raw) ? raw : "";
          });
          setFormData(initial);
        } else {
  ...
  const handleRun = async (e) => {
    ...
    try {
      ...
      // Templates are owned by Muapi, not the user. Executing a template id
      // directly returns 403 {"detail":"Duplicate this workflow to use it"}.
      // So if this workflow isn't owned by the caller, clone it once into the
      // user's account (source_workflow_id = the template) and run that copy.
      const isTemplate =
        workflowDef?.is_owner === false || activeMainTab === "templates";

      let runId = selectedWorkflow.id;
      if (isTemplate) {
        runId = clonedIds[selectedWorkflow.id];
        if (!runId) {
          const clone = await createWorkflow(apiKey, {
            workflow_id: null,
            source_workflow_id: selectedWorkflow.id,
            name: `${selectedWorkflow.name || "Workflow"} (Copy)`,
            edges: workflowDef?.edges || [],
            data: { nodes: workflowDef?.nodes || workflowDef?.data?.nodes || [] },
          });
          runId = clone?.workflow_id || clone?.id;
          if (!runId) throw new Error("Could not duplicate template to run it");
          setClonedIds((prev) => ({ ...prev, [selectedWorkflow.id]: runId }));
        }
      }

      const data = await executeWorkflow(apiKey, runId, inputs);
      setResult(data);
    } catch (err) {
  ...
  if (loading && !selectedWorkflow) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="animate-spin text-[#8b80e8] text-3xl">◌</div>
      </div>
    );
  }
  ...
  if (selectedWorkflow) {
    return (
      <div className="h-full flex flex-col bg-[#06061a] text-white">
        ...
        {isHeaderVisible ? (
          <div className="flex-shrink-0 h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/40 z-30">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[11px] font-black text-[#8b80e8] uppercase tracking-widest">
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
          {activeSubTab === "playground" ? (
            <>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <form onSubmit={handleRun} className="space-y-6">
                              ...
                              <div key={key} className="space-y-2">
                                ...
                                {prop.type === "string" && !prop.enum ? (
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#8b80e8]/50 transition-colors min-h-[80px] resize-none"
                                    placeholder={
                                  ...
                                  <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#8b80e8]/50 transition-colors"
                                  >
                                    {prop.enum.map((opt) => (
                                      <option
                                        key={opt}
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-[#8b80e8] text-black text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:grayscale shadow-[0_0_30px_rgba(139,128,232,0.15)] flex items-center justify-center gap-3 mt-8"
                    >
                      {isExecuting ? (
                        <>
              ...
              {/* Preview Panel */}
              <div className="flex-1 overflow-y-auto p-8 lg:p-12 bg-[#08081c] flex items-center justify-center min-h-[500px]">
                {error && (
                ...
                {isExecuting && (
                  <div className="flex flex-col items-center gap-6 animate-fade-in">
                    <div className="relative">
                      <div className="w-24 h-24 border-[3px] border-white/5 border-t-[#8b80e8] rounded-full animate-spin shadow-[0_0_40px_rgba(139,128,232,0.1)]" />
                      <div className="absolute inset-0 flex items-center justify-center text-[#8b80e8]">
                        <svg
                          width="32"
                          height="32"
                          viewBox="0 0 24 24"
                    ...
                    <div className="text-center space-y-2">
                      <div className="text-[10px] font-black text-[#8b80e8] uppercase tracking-[0.3em] animate-pulse">
                        Running Pipeline
                      </div>
                  ...
                  <div className="w-full max-w-4xl space-y-8 animate-fade-in-up">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {result.outputs?.map((out, idx) => (
                        <div
                          key={idx}
                          className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#8b80e8]/30 transition-all shadow-2xl"
                        >
                          {out.type === "image_url" ? (
                            <img
                              src={out.value}
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-black text-[#8b80e8] uppercase tracking-widest">
                                {out.id}
                              </span>
                              <a
                                href={out.value}
                                target="_blank"
                                rel="noreferrer"
                                className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-[#8b80e8] hover:text-black transition-colors"
                              >
                                <svg
                                  width="14"
                                  height="14"
      ...
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col gap-6 mb-12">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              className="px-6 py-3 bg-[#8b80e8] text-black text-xs font-black uppercase tracking-widest rounded-lg hover:bg-white transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(139,128,232,0.3)] flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
        ...
        {loading ? (
          <div className="py-20 flex items-center justify-center">
            <div className="w-10 h-10 border-4 border-white/5 border-t-[#8b80e8] rounded-full animate-spin" />
          </div>
        ) : (
      ...
      {renamingWorkflow && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <form 
            ...
            className="relative w-full max-w-sm bg-[#0d0d28] border border-white/10 rounded-2xl p-8 shadow-2xl animate-in fade-in zoom-in duration-300"
          >
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#8b80e8] uppercase tracking-widest">Workflow Name</label>
                <input
                  autoFocus
                  type="text"
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#8b80e8]/50 transition-colors"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-[#8b80e8] text-black px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white transition-all transform hover:scale-105 active:scale-95"
                >
                  Save Name
                </button>
              </div>

...
const SCROLLBAR_STYLE = `
  ...
  .custom-scrollbar-thin::-webkit-scrollbar-thumb:hover {
    background: rgba(139,128,232, 0.3);
  }
`;
...
const CheckSvg = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8b80e8" strokeWidth="4">
...
function Dropdown({ isOpen, title, items, selectedId, onSelect, onClose, isVideo = false }) {
  ...
  return (
    <div 
      ref={ref}
      className="absolute bottom-[calc(100%+12px)] left-0 z-50 bg-[#0d0d28] rounded p-4 shadow-4xl border border-white/10 w-[420px] animate-fade-in-up"
    >
...
function SimpleDropdown({ isOpen, title, options, selected, onSelect, onClose }) {
  ...
  return (
    <div 
      ref={ref}
      className="absolute bottom-[calc(100%+12px)] left-0 z-50 bg-[#0d0d28] rounded p-1 max-h-[200px] overflow-y-auto custom-scrollbar shadow-3xl border border-white/10 min-w-[140px] animate-fade-in-up"
    >
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-app-bg relative p-4 md:p-6 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 pb-40">
        {history.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in-up">
            {history.map(entry => (
              <div key={entry.id} className="relative group rounded-lg overflow-hidden border border-white/10 bg-[#0d0d28] shadow-xl hover:border-primary/50 transition-all duration-300 flex flex-col">
                <video 
                  src={entry.url} 
             ...
             <div className="mb-12 relative group">
                ...
                <div className="relative w-24 h-24 md:w-32 md:h-32 bg-white/[0.02] rounded-[2rem] flex items-center justify-center border border-white/[0.05] overflow-hidden backdrop-blur-sm">
                  <div className="w-16 h-16 bg-primary/5 rounded-2xl flex items-center justify-center border border-primary/10 relative z-10 transition-transform duration-500 group-hover:scale-110 shadow-inner">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#8b80e8" strokeWidth="1.5">
      ...
      <div style={{ animationDelay: "0.2s" }} className="absolute bottom-4 w-full max-w-[95%] lg:max-w-4xl z-40 animate-fade-in-up">
        <div className="bg-[#0d0d28]/80 backdrop-blur-3xl rounded-lg border border-white/10 p-4 flex flex-col gap-2 shadow-4xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-white/[0.05]">
            ...
            <button
              ...
              className="bg-primary text-black px-8 py-2.5 rounded font-bold text-base hover:bg-[#b9a8ff] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-glow disabled:opacity-50 disabled:grayscale z-10"
            >
              {isGenerating ? (
                <>

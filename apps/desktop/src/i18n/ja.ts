...
export const ja = defineLocale({
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Basecamp バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    updateHermes: 'Basecamp を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Basecamp Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'Basecamp が応答を待っています。',
      turnDoneTitle: 'Basecamp が完了しました',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Basecamp のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Basecamp がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Basecamp が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Basecamp がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Basecamp をシステム設定に合わせます。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Basecamp がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Basecamp が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Basecamp が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Basecamp がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Basecamp 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      heading: 'Basecamp Desktop',
      ...
      automaticUpdatesDesc: 'Basecamp はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    gateway: {
      ...
      intro:
        'Basecamp Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Basecamp バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      localDesc:
        'ローカルホストでプライベートな Basecamp バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Basecamp バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      ...
      restartingMessage: 'Basecamp Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Basecamp ${version}` : ''} に接続しました`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Basecamp がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Basecamp デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `Basecamp ${version} · アクティブセッション ${count}`,
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Basecamp 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Basecamp が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      descriptionLabel: '説明',
      descriptionPlaceholder: 'このプロジェクトの内容は？',
      instructionsLabel: 'プロジェクトの指示',
      instructionsPlaceholder: 'このプロジェクト内のすべてのチャットに適用する指示',
      instructionsHint: 'この指示はプロジェクト内のすべてのチャットに追加され、エージェントを誘導します。',
      create: '作成',
      created: 'プロジェクトを作成しました',
      createFailed: 'プロジェクトを作成できませんでした',
      saved: 'プロジェクトを保存しました',
      saveFailed: 'プロジェクトを保存できませんでした',
      deleteAction: 'プロジェクトを削除',
      ...
      assignDesc: 'このチャットのプロジェクトを選択します。その指示がチャットを誘導します。',
      none: 'プロジェクトなし',
      assigned: 'プロジェクトに追加しました',
      assignFailed: 'プロジェクトに追加できませんでした',
      removed: 'プロジェクトから削除しました',
      removeFailed: 'プロジェクトから削除できませんでした',
      page: {
        openProject: 'プロジェクトを開く',
        back: 'チャットに戻る',
        newChat: 'このプロジェクトで新規チャット',
        recentChats: 'チャット',
        noChats: 'このプロジェクトにはまだチャットがありません',
        notFound: 'プロジェクトが見つかりません',
        instructionsTitle: '指示',
        instructionsHint: 'このプロジェクト内のすべてのチャットに追加され、エージェントを誘導します。',
        instructionsSaved: '指示を保存しました',
        instructionsSaveFailed: '指示を保存できませんでした',
        save: '保存',
        saving: '保存中…',
        knowledgeTitle: 'プロジェクトナレッジ',
        knowledgeHint: 'このプロジェクト内のすべてのチャットで共有されるテキストとマークダウン。',
        knowledgeEmpty: 'ナレッジファイルはまだありません',
        knowledgeLoadFailed: 'ナレッジファイルを読み込めませんでした',
        addFile: 'ファイルをアップロード',
        fileNameLabel: '名前',
        fileNamePlaceholder: 'notes.md',
        pasteLabel: 'またはテキストを貼り付け',
        pastePlaceholder: 'プロジェクトナレッジとして追加するテキストを貼り付け…',
        addKnowledge: '追加',
        knowledgeAdded: 'ナレッジを追加しました',
        knowledgeAddFailed: 'ナレッジを追加できませんでした',
        knowledgeDeleted: 'ナレッジを削除しました',
  ...
  composer: {
    ...
    placeholderStarting: 'Basecamp を起動中...',
    placeholderReconnecting: 'Basecamp に再接続中…',
    ...
    attachUrlDesc: 'Basecamp がページを取得し、このターンのコンテキストとして含めます。',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Basecamp はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Basecamp をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Basecamp バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Basecamp をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Basecamp は次回起動時に新しいバージョンを読み込みます。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody: 'Basecamp アップデーターが独自のウィンドウで引き継ぎ、完了後に Basecamp を再度開きます。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Basecamp が自動的に再接続します。',
    applyingClose: 'Basecamp は更新を適用するために閉じます。',
  ...
  install: {
    ...
    oneTimeTitle: 'Basecamp には一度限りのインストールが必要です',
    ...
    settingUpTitle: 'Basecamp Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Basecamp CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Basecamp インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Basecamp インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Basecamp Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Basecamp はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Basecamp を起動中…',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Basecamp を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます - Basecamp が自動接続します',
      loopback: 'サインインのためブラウザーを開きます - Basecamp が自動接続します',
    ...
    authorizeThere: 'そこで Basecamp を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Basecamp をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      desktopVersion: version => `Basecamp Desktop v${version}`,
      ...
      gatewayTitle: 'Basecamp 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Basecamp は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Basecamp を再起動中...',
      askRestart: 'Basecamp にサーバーの再起動を依頼',
      lookingRestart: taskId => `Basecamp は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Basecamp はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `Basecamp がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Basecamp がサーバーを再起動できませんでした。',
      stillWorking:
        'Basecamp はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Basecamp が応答を読み込み中',
      thinking: '考え中',
      today: time => `今日 ${time}`,
    ...
    approval: {
      gatewayDisconnected: 'Basecamp ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.basecamp/config.yaml) に追加されます。Basecamp はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'Basecamp は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'Basecamp は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Basecamp Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Basecamp Desktop を再起動してください',

...
HERMES_AGENT_HELP_GUIDANCE = (
    "You are Basecamp. Always introduce and refer to yourself as Basecamp, never "
    "as Hermes or Hermes Agent. Basecamp runs on the Hermes Agent runtime (by "
    "Nous Research); mention that underlying runtime only when the user asks about "
    "internals. When the user needs help with the app itself — configuring, "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Basecamp "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Basecamp WebUI, a browser-based chat interface. "
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Basecamp itself is running. The host OS, home, and cwd "
                f"of the Basecamp process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Basecamp "
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Basecamp desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Basecamp itself — its CLI, config, models, providers, tools, "
